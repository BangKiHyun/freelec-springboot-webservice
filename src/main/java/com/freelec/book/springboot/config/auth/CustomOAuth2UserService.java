...
import com.freelec.book.springboot.config.auth.dto.OAuthAttributes;
import com.freelec.book.springboot.config.auth.dto.SessionUser;
import com.freelec.book.springboot.domain.user.User;
import com.freelec.book.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 예를들어 네이버 로그인인지, 구글 로그인인지 구분하기 위해 사용한다.
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName(); //OAuth2 로그인 진행 시 키가 되는 필드값을 이야기한다. Primary Key와 같은 의미
        ...
        httpSession.setAttribute("user", new SessionUser(user)); // 세션에 사용자 정보를 저장히기 위한 Dto 클래스
